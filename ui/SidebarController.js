...
export class SidebarController {
    ...
    /**
     * Renders the agent cards using the CSS classes from style.css.
     */
    ...
    async checkExistingAnalysis(agentId, card, btnElement, textSpan, iconSvg) {
        try {
            ...
            if (response.ok) {
                ...
                this._renderAnalysisResults(card, btnElement, textSpan, iconSvg, analysisData);
            }
        } catch (error) {
    ...
    _renderAnalysisResults(card, btnElement, textSpan, iconSvg, data) {
        ...
        if (!detailsDiv) {
            ...
            // Styling is now handled in CSS (.analysis-details)
            card.appendChild(detailsDiv);
        }
        detailsDiv.innerHTML = `<strong>Analysis:</strong><br>${data.justification}`;
    }
}