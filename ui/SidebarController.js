...
export class SidebarController {
    ...
    renderAgentList(agents) {
        // We maken nu ALLEEN de lijst leeg, de footer blijft staan!
        ...
        if (!agents || agents.length === 0) {
            if (this.listContainer) {
                this.listContainer.innerHTML = '<p style="color:#ccc; padding:10px;">Nog geen agents geplaatst.</p>';
            }
            return;
        }
        ...
        agents.forEach((agent, index) => {
            ...
            // AANGEPAST: Voeg toe aan de lijst container
            if (this.listContainer) {
    ...
    async runAnalysis(btnElement) {
        ...
        try {
            ...
            // AANGEPAST: Poort 3000 gebruikt
            const response = await fetch(`http://localhost:3000/api/analyze-pov`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ agentId, imageBase64: base64Image }),
                signal
            });
    ...
    async checkExistingAnalysis(agentId, card, btnElement, textSpan, iconSvg) {
        try {
            // AANGEPAST: Poort 3000
            const response = await fetch(`http://localhost:3000/api/analysis?agentId=${agentId}`);
            if (response.ok) {
    ...
    _renderAnalysisResults(card, btnElement, textSpan, iconSvg, data, agentId) {
        ...
        if (score >= 70) btnElement.style.backgroundColor = "#4caf50";
        else if (score >= 50) btnElement.style.backgroundColor = "#ff9800";
        else btnElement.style.backgroundColor = "#f44336";
        ...
        // AANGEPAST: Poort 3000
        const imageUrl = `http://localhost:3000/api/camera/${agentId}/image?t=${new Date().getTime()}`;
